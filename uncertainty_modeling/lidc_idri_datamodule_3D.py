...
class LidcIdriDataModule3D(pl.LightningDataModule):
    def __init__(
        self,
        shift_feature: str = "internal Structure",
        num_raters: int = 3,
        ...
        **kwargs,
        ...
        super(LidcIdriDataModule3D, self).__init__()
        self.shift_feature = shift_feature
        ...
        self.tr_keys = None
        self.val_keys = None
        self.id_test_keys = None
        self.ood_test_keys = None
    ...
    def prepare_data(self) -> None:
        ...
        if not os.path.exists(os.path.join(self.data_input_dir, "preprocessed")):
            print("Preprocessing data. [STARTED]")
            self.preprocess_dataset(
                root_dir=os.path.join(self.data_input_dir),
            )
    ...
    def preprocess_dataset(self, root_dir: str) -> None:
        ...
        output_dir_images = os.path.join(root_dir, "preprocessed", "images")
        output_dir_labels = os.path.join(root_dir, "preprocessed", "labels")
        ...
        for image, label, f in zip(all_images, all_labels, nii_files):
            ...
            for rater in range(len(label)):
                ...
                np.save(
                    os.path.join(
                        output_dir_labels,
                        f.split(".")[0] + "_{}_mask.npy".format(str(rater).zfill(2)),
                    ),
                    label_rater,
                )
    ...
    def create_splits(
        ...
        """
        np.random.seed(seed)
        # npy_files = subfiles(image_dir, suffix=".npy", join=False, sort=True)
        ...
        metadata_df["Segmentation Save Paths"] = metadata_df[
            ...
            lambda paths: [
                "{}.npy".format(path.split("/")[-1].split(".")[0]) for path in paths
            ]
        )
